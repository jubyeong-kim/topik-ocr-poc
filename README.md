...
#### 호출을 더 줄일 수 있을까 — 두 방법 모두 실패

페이지 단위로 5회까지 줄였으니, 더 줄일 수 있는지 두 가지를 실측했다.

| 방법 | 결과 | 원인 |
|------|------|------|
| **배치** (`images[]` 배열에 5장) | ❌ HTTP 400 | API 가 명시적으로 거부: `images=size must be between 0 and 1` — **요청당 1장만 허용** |
| **병합** (5장을 1장으로 이어붙임) | ❌ **CER 1.000** | 3024×20160 / base64 17.6MB. HTTP 200 이지만 **인식 결과 0건** |

병합은 특히 위험한 실패였다. **오류가 아니라 빈 결과로 돌아왔다.**
운영 환경이라면 "글자가 없는 이미지"와 구분되지 않아 조용히 잘못된 채점을 낼 수 있다.
→ 응답의 `inferResult` 를 확인해 처리 실패를 예외로 드러내도록 코드를 고쳤다.

> **결론: 페이지 단위 5회가 최소이자 최적이다.** 호출 수는 이미지 장수에 비례하며 더 줄일 방법이 없다.
> 이는 비용 산정의 기준이 된다 — **답안지 1장 = API 1회**.
