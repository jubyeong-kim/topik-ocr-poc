...
| 질문 | 답 |
|------|-----|
| 손글씨를 읽을 수 있는가 | ✅ **읽을 수 있다** — 최고 CER **0.010** (비전 LLM) |
| 완전 자동 채점이 되는가 | ❌ **아직 안 된다** — 행 단위 완전일치 최대 32% |
| 가장 정확한 방법의 문제는 | ⚠️ 비전 LLM 은 **학습자 오류를 고쳐 읽는다** — 채점 대상이 사라진다 |
| 그럼 무엇을 만들 수 있나 | ✅ **앙상블로 불확실 지점을 찾고, 판별로 해소** — 아래 [권장 설계](#-권장-설계) |
| 비용은 | **답안지 1장 = 3원** (무료 100건/월) |

> 📌 **핵심 발견**: 이 도메인에서는 **"잘 읽는 것"과 "쓰인 대로 읽는 것"이 다르다.**
> 쓰기 시험 채점이므로 학습자의 오류를 **보존**해야 하는데, 가장 정확한 모델일수록
> 문맥으로 **고쳐 읽는다.** 이 긴장이 설계 전체를 규정한다.

> 📍 **읽기 전에**: 이 프로젝트는 **PoC 를 지나 기술 검증·설계 단계**에 있다.
> 동작하는 제품은 없고, 표본은 53행 / 필체 5명이다.
> 숫자를 어디까지 믿어야 하는지는 [이 프로젝트는 어디까지 왔나](#-이-프로젝트는-어디까지-왔나) 참조.
...
| 방식 | CER | 일치율 | 오류 보존 | 결정적 | 비용 |
|------|-----|--------|-----------|--------|------|
| 🥇 **비전 LLM** | **0.010** | 60~80% | ⚠️ **7~8/9** | ❌ | 높음 |
| 🥈 CLOVA (페이지) | 0.037 | 32% | ✅ | ✅ | 3원 |
| 🥉 surya (행별) | 0.071 | 17% | ✅ | ✅ | 무료 |
| easyocr (행별) | 0.189 | 2% | ✅ | ✅ | 무료 |

**정확도 1위가 곧 최선이 아니다.** 비전 LLM 은 가장 잘 읽지만
`이였다`(학습자 오류)를 `이었다`로 **고쳐 읽어** 채점 대상을 지운다.

> 전통 OCR 엔진 사이에서는 **순위가 입력 형태와 무관하게 동일**했다(CLOVA > surya > easyocr).
> 다만 **최적 입력은 엔진마다 다르다** — CLOVA 는 페이지 통째, easyocr·surya 는 행별 분할.
...
### 6️⃣ 잘 읽는 능력이 곧 고쳐 읽는 능력이다

비전 LLM 은 CER 0.010 으로 압도적이지만, **학습자가 틀리게 쓴 것을 맞게 읽어버린다.**

| 이미지에 쓰인 것 | LLM 전사 | 문제 |
|------------------|----------|------|
| `이였다` | `이었다` | 맞춤법 오류가 사라짐 |
| `오라하지` | `오라 하지` | 띄어쓰기 오류가 사라짐 |

쓰기 시험 채점에서는 **오류가 곧 채점 대상**이다. 지워지면 학습자는 틀리고도 만점을 받는다.

**그런데 전사가 아니라 판별을 시키면 보존된다.**

> "CLOVA 는 `이였다`, surya 는 `이었다` 로 읽었다. **어느 쪽이 이미지와 일치하는가?**"
> → LLM: **`이였다`** ("모음에 가로획이 두 개(ㅕ)라 학습자가 그렇게 썼다")

| | 전사 (생성) | 판별 (선택) |
|---|-------------|-------------|
| 출력 | 자유 텍스트 | A 또는 B |
| 고쳐 쓸 여지 | 있음 | **없음** |
| 정확도 | CER 0.010 | **92%** (12건 중 11) |

---

## 🏗️ 권장 설계

세 발견을 조합하면 다음 구조가 나온다.
...
flowchart TD
    ...
    B --> D{"어절 대조"}
    C --> D
    D -->|"합의 60%<br/>정확도 96%"| E["✅ 그대로 채택"]
    D -->|"불일치 40%"| F{"A/B 판별 가능?"}
    F -->|"37%"| G["🤖 LLM 객관식 판별<br/>92% · 오류 보존"]
    F -->|"63%"| H["👤 사용자 확인"]
    E --> I["📝 최종 텍스트"]
    G --> I
    H --> I
    style E fill:#e0ffe0,stroke:#3a3
    style G fill:#e0f0ff,stroke:#36a
    style H fill:#fff0e0,stroke:#d83
```
...
| 단계 | 왜 | 측정값 |
|------|-----|--------|
| CLOVA + surya **병렬** | 서로 다른 실수를 하므로 불일치가 곧 위험 신호 | 합의 96% / 불일치 33% |
| 합의 구간은 그대로 | 두 엔진이 같으면 믿을 만하다 | 미표시 정확도 96% |
| 불일치는 **LLM 판별** | 객관식이라 오류를 고쳐 쓰지 못한다 | 92%, 오류 보존 |
| 나머지는 **사람 확인** | 정렬이 어긋나거나 둘 다 틀린 경우 | 불일치의 63% |

**특성**

- 지연: 두 엔진이 병렬이므로 느린 쪽 하나만큼(약 1.3초) + 판별 호출
- 비용: 3원 + surya 무료 + 불일치분 LLM
- 결정성: CLOVA 10/10 동일 확인 → **항상 같은 곳에 표시**된다
- 자동화율: 전체의 약 **77%** (합의 60% + 판별 가능 17%)

⚠️ 이 설계는 **구성 요소를 각각 검증한 것**이고, 파이프라인 전체를 통합 측정하지는 않았다.
...
|------|------|------|
| 1. **PoC** | "되기는 하나?" | ✅ 통과 |
| 2. **기술 검증·설계** | "어떻게 해야 되나?" | ✅ **현재 위치** |
| 3. 프로토타입 | "동작하는 물건이 있나?" | ❌ |
| 4. MVP | "사용자가 쓰나?" | ❌ |

### PoC 를 넘어선 부분

PoC 라면 엔진 하나를 돌려 "된다/안 된다"로 끝났을 것이다. 실제로는 그 이상을 했다.

| 수행한 것 | 성격 |
|-----------|------|
| 엔진 4종을 **동일 벤치마크**로 비교 | 비교 실험 |
| 입력 형태 3가지 실험 (행 / 페이지 / 병합) | 설계 탐색 |
| 교란 변수 통제 (어노테이션 박스·여백·해상도) | 실험 설계 |
| 오류를 **자모 단위로 유형화** | 원인 분석 |
| **도메인 제약 발견** — "오류 보존" | 문제 정의의 확장 |
| 아키텍처 설계 + 구성요소별 검증 | 기술 설계 |

특히 **"학습자 오류를 보존해야 한다"는 제약은 일반 OCR 문헌에 없다.**
쓰기 채점이라는 도메인에서만 생기는 요구이며, 이를 발견하고 지표화하고
해법(객관식 판별)까지 검증한 것이 이 프로젝트의 고유한 기여다.

### 아직 프로토타입이 아닌 이유

| 없는 것 | 의미 |
|---------|------|
| **통합 파이프라인** | 설계도는 있으나 구현이 없다. 조각별로만 측정했다 |
| **앱 연결** | `topiq-write` 와 아직 붙지 않았다 |
| **실사용자** | 0명 |
| **운영 요소** | 에러 처리·모니터링·확장성 미고려 |

`poc.py` 는 **측정 도구이지 제품이 아니다.**

### 숫자의 신뢰 구간을 낮게 잡아야 한다

| 항목 | 값 |
|------|-----|
| 실제 손글씨 | 53행 / 필체 5명 |
| 필자 국적 | **전원 한국인** (타겟은 중국어권 학습자) |
| 데이터 출처 | 공개 데모 1종 |

지표는 촘촘히 뽑았지만 **기반 데이터가 얇다.**
0.037 같은 소수 셋째 자리를 논했으나, 53행 표본에서 그 자릿수는 신뢰하기 어렵다.
**순위와 배수 차이는 유효하되, 미세 차이는 참고값으로 볼 것.**

### 가장 값진 산출물은 결과가 아니라 도구다

```bash
엔진을 바꿔볼까?      → poc.py run --engine <새엔진>
입력 형태를 바꿔볼까? → poc.py pagecompare
그래서 뭐가 틀렸지?   → analyze_errors.py
```

새 모델이 나와도 **같은 기준으로 즉시 비교**된다.
하루 만에 easyocr → surya → CLOVA → 비전 LLM 을 옮겨 다닐 수 있었던 것이 이 덕분이다.

> **과제 기준으로는 요구를 넘었고, 상용화 판단 기준으로는 아직 부족하다.**
> "이 설계가 실제로 77% 자동화를 낸다"는 것은 **구성요소 측정에 근거한 추정**이며
> 통합 측정으로 확인해야 한다 → [다음 단계](#-다음-단계) 1순위.
...
| 6 | **정답은 사람 판독** | 실제로 1건(`폼출할`→`표출할`)이 오독이었고 **LLM 이 잡아냈다**. 전사 오류가 더 있을 수 있다 |
| 9 | **비전 LLM 은 비결정적** | 같은 이미지에 다른 결과가 나올 수 있다. CLOVA 는 10/10 동일함을 확인했으나 LLM 은 미검증 |
| 10 | **권장 설계 미통합** | 구성 요소는 각각 쟀으나 파이프라인 전체를 한 번에 측정하지 않았다 |
| 11 | **판별 가능 구간이 37%** | 불일치의 63%는 A/B 선택으로 풀리지 않아 사람이 원본을 봐야 한다 |
...
| 우선순위 | 과제 | 비용 |
|----------|------|------|
| **1** | **권장 설계 통합 측정** — 파이프라인 전체를 한 번에 재기 | 2~3일 |
| **2** | **사용성 검증** — 초안 교정이 백지 타이핑보다 빠른가 | 1일 |
