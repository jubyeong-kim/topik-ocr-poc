...
📄 문제 정의서: **[PROBLEM.md](PROBLEM.md)** · 🗺️ 다음 단계 제안서: **[NEXT_STEPS.md](NEXT_STEPS.md)** · 🔁 회고: **[RETROSPECTIVE.md](RETROSPECTIVE.md)**
...
#### 그런데 띄어쓰기 교정은 **만들지 않기로 했다** — 상한을 재보니

"띄어쓰기만 고치면 크게 오르겠다"는 기대로 후처리를 만들려다, 먼저 **얻을 수 있는 상한**을 쟀다.

| 구분 | 건수 |
|------|------|
| 띄어쓰기 포함 완전일치 | 7/53 |
| **띄어쓰기 무시 일치** | **17/53** ← 공백을 100% 고쳤을 때의 상한 |
| 글자 자체가 틀린 행 | **36/53** ← 띄어쓰기 교정으로는 못 고침 |

**상한이 32% 다.** 그런데 이 값은 이미 우리가 보고하는 "공백 무시 일치율"과 같은 수치다.
즉 **띄어쓰기 교정의 이득은 이미 측정되어 있었고, 새 코드로 얻을 것이 없다.**
진짜 병목은 남은 **36건의 글자 오인식**이다.

게다가 도메인과 충돌한다:

> TOPIK 은 **띄어쓰기도 채점 대상**이다. 한국어 띄어쓰기 모델로 OCR 출력을 교정하면
> **학습자가 실제로 낸 띄어쓰기 오류까지 고쳐 버린다.** 채점해야 할 오류를 지우는 셈이다.
> 반대로 교정하지 않으면 OCR 이 만든 오류를 학습자 잘못으로 채점한다.

**결론**: OCR 경로에서는 **띄어쓰기 항목을 신뢰할 수 없다.**
제품에서는 띄어쓰기 채점을 제외하거나 "OCR 불확실" 로 표시하는 것이 정직하다.
이는 코드로 풀 문제가 아니라 **제품 설계로 정해야 할 문제**다.
...
| PaddleOCR | ⚠️ **실행 실패** | 아래 별도 항목 참조 — **정확도는 측정하지 못했다** |
...
#### PaddleOCR — 실행에 실패했다 (정확도 미측정)

처음에는 "Windows 설치가 까다롭다"는 이유로 기각했으나, **측정은 전부 Colab(Linux)에서 하므로
그 사유는 적용되지 않는다**는 지적을 받고 재시도했다. 온프레미스 후보를 하나라도 더 확보하는 것은
개인정보 제약(§5-1) 상 가치가 있다.

| 시도 | 결과 |
|------|------|
| 1. 기존 세션(torch 설치됨)에 설치 | **Segmentation fault** — 라이브러리 충돌 의심 |
| 2. 깨끗한 세션에 paddle 만 설치 | `NotImplementedError: ConvertPirAttribute2RuntimeAttribute not support` (oneDNN) |
| 3. oneDNN·PIR 실행기 비활성화 후 재시도 | 동일 오류 |

설치(`pip install paddlepaddle paddleocr`)와 API 확인(3.7.0, `predict`/`ocr` 모두 존재)까지는 성공했으나
**추론 단계에서 실패**했다. paddlepaddle CPU 빌드의 oneDNN 백엔드와 새 PIR 실행기 간 비호환으로 보인다.

> ⚠️ 이는 **"PaddleOCR 이 부정확하다"는 뜻이 아니다.** 정확도는 재보지도 못했다.
> 다만 **운영 관점의 마찰이 크다**는 것은 확인된 사실이며, 이는 온프레미스 후보로서 실질적인 감점 요인이다.

다시 시도한다면: `paddlepaddle-gpu` 사용, `paddleocr 2.x` 로 다운그레이드,
또는 paddlepaddle 버전 고정을 순서대로 검토할 만하다.
...
2. **최적 입력 형태가 엔진마다 다르다.** CLOVA 는 페이지 통째, easyocr·surya 는 행별 분할이 유리하다.
   따라서 **엔진을 교체하면 전처리 파이프라인도 함께 재측정**해야 한다.
...
- [x] ~~엔진 3종을 페이지 단위로 재비교~~ → 완료. **페이지 이점은 CLOVA 고유**, 엔진 순위는 불변
- [x] ~~띄어쓰기 교정 후처리~~ → **검토 후 기각.** 상한이 32%(이미 측정된 값)이고,
      한국어 띄어쓰기 모델은 **학습자의 실제 띄어쓰기 오류까지 지워** 채점과 충돌한다
- [x] ~~호출 수 최적화~~ → 완료. **여백 제거 + 50% 축소 병합으로 5회 → 1회, 정확도 유지**
- [x] ~~여백 제거 효과 검증~~ → 완료. **단일 페이지엔 영향 없음**(5장 재측정).
      병합할 때만 필수 — 전처리 정책 확정
- [ ] **클라이언트 축소 정책 확정** — 453px 폭에서도 CER 0.042 였다. 표본을 늘려
      안전한 최소 해상도를 정하면 업로드 용량을 대폭 줄일 수 있다
- [x] ~~글자 오인식 36건 분석~~ → 완료. **삭제 46%, 초성 치환 52%, 획 손실 방향으로 편향.
      단일 혼동 교정으로 복구되는 행은 0건**(행당 평균 1.7건이 겹침)
- [ ] **띄어쓰기 채점 정책 결정** — OCR 경로에서는 신뢰 불가. 제외할지 "불확실" 표시할지는 제품 결정
