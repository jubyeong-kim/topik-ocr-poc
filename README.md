...
```
CER 0.092  →  100자당 약 9자 틀림  →  91% 정확
CER 0.222  →  100자당 약 22자 틀림 →  78% 정확
```
...
| **0.05~0.10** | **초벌 전사로 쓸만함. 사람 교정 필요** | 실제 육필 surya **0.092** |
| 0.10~0.20 | 읽히긴 하나 교정 부담이 큼 | |
| 0.20~ | 실용 어려움 | 실제 육필 easyocr **0.222** |
...
CER 은 3배 차이인데 일치율은 83% → 9% 로 무너진다. 이유는 단순하다.

> **CER 은 평균이고, 완전일치는 "한 글자도 안 틀려야" 성립한다.**
> 40자 문장에서 CER 0.092 면 평균 **3.7자**가 틀린다. 0자일 확률은 희박하다.
...
**공정한 비교는 CER 로 봐야 하며, 그 값도 0.029 vs 0.222 로 8배 차이다.**
