...
### 🏆 핵심 결과 — 실제 손글씨 53행 / 필체 5종, 엔진 2종 비교

**측정 조건**: [입력 샘플 B](#b-실제-손글씨--레포에-미포함-53행--필체-5종) · 두 엔진 모두 **Colab T4** · 후처리·정답·지표 계산 동일

| 지표 | easyocr | **surya** | 목표 |
|------|---------|-----------|------|
| 평균 CER (전체 53행) | 0.196 | **0.102** | 0.15 이하 |
| **평균 CER (단편 제외 51행)** | 0.189 | **0.071** | 0.15 이하 → ✅ **surya 달성** |
| CER (띄어쓰기 무시) | 0.193 | **0.096** | 참고 |
| 공백 무시 일치율 | 1/53 (2%) | **9/53 (17%)** | 80% 이상 → ❌ 둘 다 미달 |
| 처리 시간 (T4) | 0.15초/행 | 1.34초/행 | — |

**엔진 교체만으로 CER 이 62% 감소**했다(0.189 → 0.071). 파인튜닝도, 데이터 추가도 없다.
대신 surya 가 **9배 느리다** — 정확도와 속도의 교환이다.
...
| 009 | 0.188 | 0.065 |
| 010 | 0.190 | 0.083 |
| **편차** | 0.181~0.226 (**1.2배**) | 0.065~0.157 (**2.4배**) |

easyocr 은 **모든 필체에서 고르게 나쁘고**, surya 는 **필체를 탄다.**
가장 잘 읽은 필체(009, 0.065)와 가장 못 읽은 필체(002, 0.157)가 2.4배 차이다.
→ 실사용 시 **필체에 따라 체감 품질이 크게 달라진다**는 뜻이며, 단일 필체로 성능을 주장하면 안 된다.
...
1. **완전 자동 채점은 아직 불가.** 최고 성능(surya)에서도 일치율 17% 로, 정답 문자열 비교 방식의
   51/52번 자동채점에는 못 쓴다. CER 목표는 달성했지만 일치율 목표는 여전히 미달이다.
2. **필체 편차가 크다.** surya 는 필체에 따라 CER 이 **2.4배(0.065~0.157)** 차이 난다.
...
- [ ] **B 전략 사용성 검증** ← 최우선.
      CER 0.071 초안을 고치는 것이 백지 타이핑보다 실제로 빠른지 측정. **제품 방향 자체의 타당성 판정**
- [ ] **채점 결과 일치율 측정** — `PROBLEM.md` 가 정의한 최종 판정 기준이나 아직 미측정.
      글자가 틀려도 채점 결과가 같으면 실용상 충분하므로 일치율 17% 보다 좋게 나올 수 있다
- [ ] 외국인 학습자 필체 확보 — 현재 표본은 전원 한국인이며, 실제 타겟과 다르다
- [ ] surya 를 한글 손글씨로 파인튜닝 — CER 0.071 에서 얼마나 더 내려가는지
- [ ] 필체 편차(2.4배) 원인 분석 — 어떤 필체 특성이 인식률을 떨어뜨리는지
